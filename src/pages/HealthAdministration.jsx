  ...
  const evaluateHealth = () => {
    ...
    if (!isNaN(sys)) {
      if (sys < 90)
        ...
        r["الضغط الانقباضي"] = {
          status: "طبيعي",
          tips: ["حافظ على نمط حياة صحي.", "قلل التوتر والضغط النفسي."],
        };
      else
        r["الضغط الانقباضي"] = {
          status: "مرتفع",
    ...
    if (!isNaN(hr)) {
      if (hr < 60)
        ...
        r["معدل ضربات القلب"] = {
          status: "طبيعي",
          tips: ["حافظ على النشاط البدني المعتدل.", "تجنب التوتر والقلق."],
        };
      else
        r["معدل ضربات القلب"] = {
          status: "مرتفع",
    ...
    if (!isNaN(chol)) {
      if (chol < 200)
        r["الكوليسترول الكلي"] = {
          status: "طبيعي",
          tips: ["حافظ على نظام غذائي متوازن.", "مارس النشاط البدني بانتظام."],
        };
      else if (chol <= 239)
        r["الكوليسترول الكلي"] = {
          status: "حدودي",
    ...
    if (!isNaN(temp)) {
      if (temp < 36)
        ...
        r["درجة الحرارة"] = {
          status: "طبيعية",
          tips: ["لا توجد إجراءات خاصة مطلوبة.", "حافظ على نمط حياة صحي."],
        };
      else
        r["درجة الحرارة"] = {
          status: "مرتفعة",
              ...
              <span
                className={`${styles.status} ${
                  data.status === "طبيعي"
                    ...
                    : data.status === "منخفض" || data.status === "منخفضة"
                    ? styles["status-low"]
                    : data.status === "مرتفع"
                    ? styles["status-high"]
                    : styles["status-normal"]
